...
CalcIndLikelihood <- function(data, individual, initial_probabilities,transition,class_matrix,pi){
  ...
  forward <- ForwardIterFull(data,2,individual,initial_probabilities,transition,class_matrix)
  backward <- BackwardIterFull(data,2,individual,transition,class_matrix)
  ...
  likelihood <- (pi * CalcProdInd(data,individual,class)) + ((1 - pi) * individual_likelihood)
  # if (likelihood == 0){
  #   print(individual)
  # }
  return(likelihood)
...
```{r}
CalcInitialPatternBackup <- function(data, initial_probabilities, transition, class_matrix, freq_vec, pi_0){
...
```{r}
CalcTransitionPatternGeneral <- function(data, tran,class,freq_vec, pi_0, forw, backw, likelihoods){
...
```{r}
CalcClassificationPatternBackup <- function(data,init,tran,class, freq_vec, pi_0){
  k <- numOfStates(data)
