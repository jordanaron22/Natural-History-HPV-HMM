...
IntroducePersistence <- function(hpv){
  for (i in 1:dim(hpv)[1]){
    for (j in 1:dim(hpv)[2]){
      if (!is.na(hpv[i,j])){
        ...
        else if (j == 2){hpv[i,j] <- persitenceHelper(hpv[i,j], hpv[i,j-1], NA)}
        else {hpv[i,j] <- persitenceHelper(hpv[i,j], hpv[i,j-1], hpv[i,j-2])}
      }
    }
  }
  ...
  for (i in 1:dim(hpv)[1]){
    for (j in 1:dim(hpv)[2]){
      if (!is.na(hpv[i,j])){
        if (hpv[i,j] < 4){hpv_pers[i,j,1] <- hpv[i,j]}
        else if (hpv[i,j] == 4){
          hpv_pers[i,j,1] <- 1
          hpv_pers[i,j,2] <- 2
        }
...
persitenceHelper <- function(hpv_val, one_back, two_back){
  if (hpv_val == 0){return(0)}
    ...
    else if (one_back == 2){return(3)} 
    else if (one_back == 3){return(3)}
    else if (one_back == 4){return(6)}
    else if (one_back == 5){return(6)}
    else if (one_back == 6){return(3)}
    else if (one_back == 7){return(6)}
  }
  
  else if (!is.na(two_back)){
    if (two_back == 0){return(4)}
    else {return(5)}
  }
