...
```{r}
init_hc <- c(.75,.25,0)
tran_hc <- t(matrix(c(.6,.3,.1,.2,.7,.1,0,0,1),3,3))
class_hc <- t(matrix(c(.8,0.17,.03,.1,.85,.05,0,.18,.82),3,3))
```
...
init <- c(.85,.14,.01)
tran <- t(matrix(c(.75,.24,.01,.25,.7,.05,0,0,1),3,3))
  ...
  for (ind in 1:dim(data)[1]){
    ...
    for(time in 1:dim(data)[2]){
      ...
      for (u in 1:length(forward)){
        ...
        for (v in 1:length(backward)){
          # individual_likelihood <- (forward[[u]][[time]] %*% backward[[v]][[time]])[1]
          if (!is.na(x_val)){
            ...
            if (u == length(forward) & v == length(backward)){
              ...
              num_matrix_full <- num_matrix_full + num_matrix
            }
...
# 
# 
old_likelihood <- CalcLikelihoodPattern(pattern_data,init,tran,class,freq_vec)
# init <- CalcInitialPattern(pattern_data,init,tran,class,freq_vec)
# tran <- CalcTransitionPattern(pattern_data,init,tran,class,freq_vec)
class <- CalcClassificationPattern(pattern_data,init,tran,class)
new_likelihood <- CalcLikelihoodPattern(pattern_data,init,tran,class,freq_vec)
