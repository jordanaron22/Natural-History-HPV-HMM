...
class_hc <- t(matrix(c(.875,.125,0,.05,.95,0,0,.1,.9),3,3))
```
...
```{r}
IntroNA <- function(data, obs, class){
      ...
      if (!is.na(data[i,j])){
        if (obs == TRUE){
          if (rbinom(1,1,2/5) == 1){
            if (rbinom(1,1,3/5) == 1){
              if (data[i,j] == 0){
                if (runif(1) < (class[1,1] + class[1,2])){
                  data[i,j] <- 3
                } else {
                  data[i,j] <- 4
                }
              } else if (data[i,j] == 1){
                if (runif(1) < (class[2,1] + class[2,2])) {
                  data[i,j] <- 3
                } else {
                  data[i,j] <- 5
                }
              } else {
                if (runif(1) < (class[3,2] + class[3,3])) {
                  data[i,j] <- 5
                } else {
                  data[i,j] <- 4
                }
...
data_obs_NA <- IntroNA(data_obs,T, class_hc)
...
```{r}
CalcIndLikelihood <- function(data, individual, initial_probabilities,transition,class_matrix){
...
```{r}
CalcClassificationPattern <- function(data,init,tran,class){
  k <- numOfStates(data)
  num_matrix <- matrix(0L, nrow = length(k), ncol = length(k))
  
  for (ind in 1:dim(data)[1]){
    individual_likelihood <- CalcIndLikelihood(data,ind,init,tran,class)
    for(time in 1:dim(data)[2]){
      ...
      beggining_backw <- GetAllPossibilitiesBackw(data,time,ind)
      beggining_forw <- GetAllPossibilitiesForw(data,time,ind)
      x_vals_backw <- unique(beggining_backw[1,])
      x_vals_forw <- unique(beggining_forw[time,])
      
      for (u in 1:length(forward)){
        x_val_forw <- beggining_forw[time,u]
        
        for (v in 1:length(backward)){
          x_val_backw <- beggining_backw[1,v]
          
          x_vals <- x_vals_forw
          x_val <- x_val_forw
          if (!is.na(x_val)){
            
            mover <- forward[[u]][[time]]*backward[[v]][[time]] / individual_likelihood
            
            for (y_state in 1:length(k)){
              mover[y_state] <- mover[y_state] * ClassificationRatio(x_val,x_vals,y_state - 1, class)
              #mover[y_state] <- mover[y_state] * Classification(x_val,y_state - 1, class)
...
```{r}
CalcClassificationPattern(pattern_data,init_true,tran_true,class_true)
(class <- CalcClassificationPattern(pattern_data,init_true,tran_true,class_true))
(class <- CalcClassificationPattern(pattern_data,init_true,tran_true,class))
(class <- CalcClassificationPattern(pattern_data,init_true,tran_true,class))
(class <- CalcClassificationPattern(pattern_data,init_true,tran_true,class))
```
