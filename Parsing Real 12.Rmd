...
setwd("Data/Real/12/Everything from Iden/")
states <- 12
...
second <- likelihood_array[which(likelihood_array < -15641)]
second <- which.max(second)
...
GenerateInitTable(init_est_array[1,,first], sqrt(init_var), "Initial (Median")

GenerateTable(tran_est_array[,,first], sqrt(tran_var),"Transition (Median)")

GenerateTable(class_est_array[,,first], sqrt(class_var),"Classification (Median)")
```
